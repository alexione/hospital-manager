    ...
    <v-main class="bg-grey-lighten-4">
      <v-container fluid>
        <v-card class="elevation-2 rounded-lg">
          ...
          <v-data-table :headers="headers" :items="employees" :loading="loading" :search="search" class="pa-2">
            ...
            <template v-slot:item.actions="{ item }">
              ...
              <v-btn icon="mdi-delete" variant="text" color="red" size="small" @click="deleteItem(item)" :disabled="item.id === authStore.user?.id"></v-btn>
            </template>
          </v-data-table>
        </v-card>
        ...
        <v-dialog v-model="dialog" max-width="500px">
          <v-card>
            <v-card-title><span class="text-h5">{{ isEditing ? 'Editare Angajat' : 'Adaugă Angajat Nou' }}</span></v-card-title>
            <v-card-text>
              <v-container>
                <v-form ref="form" @submit.prevent="save">
                  ...
                  <v-text-field v-model="editedItem.email" label="Email" :disabled="isEditing" required type="email"></v-text-field>
                  <v-text-field v-if="!isEditing" v-model="editedItem.password" label="Parolă" required type="password"></v-text-field>
                  <v-select v-model="editedItem.role" :items="['Admin', 'Medic', 'Asistent', 'Recepție']" label="Rol" required></v-select>
                </v-form>
              </v-container>
            </v-card-text>
            <v-card-actions>
